...
For such a system the discrete IH-LQR algorithm could minimize a cost function of the type:

\begin{equation} \label{eq:lqr_cost_fcn}
	J = \sum_{k=0}^{\infty}\left( x(k)^TQx(k) + u(k)^TRu(k) + 2x(k)^TNu(k) \right)
\end{equation}
...
The cross-term is ignored in this case by setting $N=0$. For such a cost function the squared of the states and control signals, multiplied with the weights Q and R, are minimized. Each element in the diagonal elements of Q and R are the costs associated with each state or input in the state and input vectors.\\

In matlab the lqr(A,B,Q,R, N) function is used to find the controller gain $(K)$.

Inherently since LQR is a regulator it will always drive the states and inputs to zero. From the linearisation in \cref{sec:mod_lin} it was shown that the A, B and Bd matrices were the Jacobian matrices. In \cref{eq:taylor} it is obvious that the continuous model is approximated from the deviation of the states, inputs and disturbances from the operating points. Thus the cost function with no cross-term cost becomes:

\begin{equation} \label{eq:lqr_cost_fcn}
	J = \sum_{k=0}^{\infty}\left( \bar{x}(k)^TQ\bar{x}(k) + \bar{u}(k)^TR\bar{u}(k) \right)
\end{equation}

with $\bar{x} = x-x_o$ and $\bar{u} = u-u_o$ where $x_o$, $u_o$ are the linearisation equilibrium operating points of the states and inputs. In conclusion the LQR algorithm will keep the system close to the operating points. This is very appealing because of the linear system only being a good approximation exactly at the operating point.
