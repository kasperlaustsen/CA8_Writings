...
The evaporator contains one of the greater thermodynamic capacitances due to the large mass of metal in the heat exchanger. Therefore the temperatures of the evaporator vapor-liquid and vapor metal CVs are modeled by the two dynamic equations below respectively. The metal temperature change is governed by the heat flows to and from the metal ($ Q_{aml}, Q_{ml}, Q_{mvml} $ in \cref{eq:evap_dT_ml} and $ Q_{amv}, Q_{mv}, Q_{mvml} $ in \cref{eq:evap_dT_mv}) and the mass of the metal in that CV ($M_m \cdot \sigma$ in \cref{eq:evap_dT_ml} and $M_m \cdot (1 - \sigma)$ in \cref{eq:evap_dT_mv}) multiplied with the specific heat capacity of the metal ($Cp_m$).

% ALTERNATIVE MORE COMPACT WAY OF WRITING THE ABOVE:
%The metal temperature change is governed by the heat flows to and from the metal ($Q$) and the mass of the metal in that CV ($M_m \cdot \sigma$ and $M_m \cdot (1 - \sigma)$) multiplied with the specific heat capacity of the metal ($Cp_m$).

\begin{align}
	\frac{dT_{ml}}{dt} & = \frac{Q_{aml}-Q_{ml} + Q_{mvml}}{M_m \cdot \sigma \cdot Cp_m}        \label{eq:evap_dT_ml} \\
	\frac{dT_{mv}}{dt} & = \frac{Q_{amv} - Q_{mv} - Q_{mvml}}{M_m \cdot (1 - \sigma) \cdot Cp_m } \label{eq:evap_dT_mv}
\end{align}
