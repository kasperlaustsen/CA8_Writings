...
\medskip
The temperatures of the air which is blown over the evaporator is modeled before and after the evaporator by the two equations below. The fan has some loss in the form of heat which is transferred to the air. 
...
\medskip
The heat flow from air to metal of evaporator is modeled based on the assumption that the mass flow of air is cooled down to the metal temperature in \cref{eq:Q_amv} and \cref{eq:Q_aml}. 
...
\medskip
The temperatures of the two evaporator metal CVs are given by the two equations below respectively.
...
\medskip
Output pressure, specific enthalpies and mass balances are given by equations \cref{eq:evap_pout} $\rightarrow$ \cref{eq:evap_dMvdt}. \cref{eq:evap_Tsup} describes the temperature of the air leaving the evaporator and \cref{eq:evap_mdot_lv} describes the flow from the liquid-vapor CV to the vapor CV.
...
\begin{equation} \label{eq:consump_x}\renewcommand{\arraystretch}{2.5}
	...
	\begin{bmatrix}
		...
		\dot{m}_{1} + \dot{m}_{3} \\													%pjj
		 ...
		 \Gamma(T_{2},p_{int1}) \\														%cs2
		 \left(\dfrac{p_{int2}}{p_{int1}}\right)^{\frac{-1}{\gamma}} \\					%cs2
