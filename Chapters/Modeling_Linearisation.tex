A general state space system is typically defined on the form seen in \cref{eq:state_space}. It is observed that a change in states over time is defined as a linear combination of vectors and matrices. Having a model on such a state space form is a necessary for state-space controller design.

In order to make a state space controller a model on the typical state space form such as the one seen in \cref{eq:state_space} is necessary. In the previous section a non-linear model was made by substituting the algebraic constraint equations into the state derivative vector. Now, in order to reach a model on the necessary form, a linearisation is necessary. This section will explore how a first order Taylor expansion can be utilized to get the \textit{A}, \textit{B}, \textit{$B_d$} and \textit{C} matrices of the state space system representation.
...
As mentioned a first order Taylor expansion is used to achieve the desired linear system of equations and in the following the procedure is described.\\
...
By now it is apparent that a set of operating points are vital for the linearisation to work and thus these must be found. They are found by/from.... Eramus... Take it away my boiiiiii!
