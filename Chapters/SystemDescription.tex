...
\end{itemize}
Now that the principles of the BITZER refrigeration system is described, the load of the refrigeration system can be introduced. \\

\textbf{Refrigeration load}\\
The load for the BITZER refrigeration system is an insulated trailer with a cargo of 500 kg of \todo[inline]{CARGO - BANANAS?}. The air inside the trailer is cooled via the evaporator of the refrigeration system. The air mass flow rate is driven by the evaporator fan. The heat flows from and to the trailer is illustrated in \cref{fig:trailer_airflow} 





control inputs can be introduced.\\

\textbf{Control inputs}\\
The system components have variables that can be set to control their behavior. These are the control inputs:
\begin{itemize}
...
\textbf{Exogenous inputs}\\
Exogenous inputs are the inputs of a system that can't be controlled. They are commonly referred to as disturbances. The system refrigeration system has one such disturbance, namely the ambient temperature. The ambient temperature affects the heat transfer from ambient air to the air inside the reefer trailer. Additionally, it also affects the condenser heat transfer.\\
