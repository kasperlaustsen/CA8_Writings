...
The second compressor stage is depending on temperature and thus it is calculated in the pipe joining junction. In \cref{eq:PJJ_Tout} the output temperature is found from the output enthalpy and the input pressure. It is assumed that the pressure is constant throughout the junction and thus $p_{out} = p_{in}$.
