	...
	\node [tmp, right of=rinput, node distance = .5cm] (tmpadd1){};
	\node [tmp, above of=tmpadd1, node distance = 2.5cm] (tmpadd2){};
	\node [block, right of=tmpadd2, node distance = 1cm] (omegaA) {$\omega_A$};
	\node [block, right of=omegaA, node distance = 1.5cm] (deltaA) {$\Delta_A$};
%	\node [sum, right of=tmpadd1] (sum1) {};
	\node [block, right of=tmpadd1, node distance = 2cm] (G) {G};
	\node [sum, right of=G, node distance =1.5cm] (sum2) {};
	\node [tmp, above of=sum2, node distance = 2.5cm] (tmpadd3){};
...
%	
	\draw[thick, dotted] ($(G.north west)+(-1.8, 3)$) rectangle  ($(G.south east)+(1.5, -0.25)$);
